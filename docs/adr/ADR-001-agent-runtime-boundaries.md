...
数据库保存持久化事实、幂等执行权和恢复依据，是最终事实源。未来 Redis 只承担热点状态、实时协调和事件快速通道；Redis 数据丢失不能破坏正确性。

### 4. 请求幂等执行权

`requestId` 是平台级全局唯一标识，数据库 `UNIQUE(request_id)` 是最终执行权判据。未来 Redis 只做快速查询：Redis 未命中时，仍以数据库 INSERT 成功或唯一约束冲突决定执行权，再回填 Redis。

### 5. Session Lock 生命周期

未来 Session Lock 只保护活跃执行片段。进入 `WAITING_APPROVAL` 前必须保存状态/Checkpoint 并释放 Lock；恢复时重新获取 Lock，通过 CAS 或状态校验确认仍可恢复后继续。本阶段不实现 Lock、Checkpoint 或 Resume。
...
`ToolExecution.idempotencyKey` 在 V1 契约中是平台生成的全局唯一标识，推荐使用 UUID 或带全局命名空间的值，不是 Tool 自己生成的局部计数器。因此保留 `UNIQUE(idempotency_key)`。
...
项目保留 WebFlux。JPA、POI 和其他阻塞 IO 接入响应式调用链时，必须隔离到 `boundedElastic` 或专用阻塞执行器。当前持久化基础层尚未接入 Agent Runtime 的响应式执行链路。

### 9. Prompt Injection 安全口径

项目只能宣称提供提示词注入缓解措施，不能宣称该问题已彻底解决或 100% 安全。真实执行边界由 Schema Validation、PolicyEngine、未来的 RBAC、审批和业务规则共同构成。

### 10. Flyway / Hibernate 过渡状态

Runtime 表的创建与版本化以 Flyway Migration 为规范来源。由于旧版 Schema 仍依赖 `ddl-auto=update`，同一持久化单元下 Hibernate 当前仍会检查 Runtime Entity 对应的 Schema；这是 V1 明确接受的过渡状态，不宣称 Flyway 已是所有表的唯一 Schema 权威来源。

后续 Schema Migration Cleanup 阶段将把旧版表纳入 Flyway，并把 `ddl-auto` 改为 `validate`，届时 Flyway 才成为唯一 Schema 权威来源。该过渡状态不阻塞 Phase 4。
